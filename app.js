/* ============================================================
   BLOCK JUMP  v1.2.0  —  app.js  (Part 1 / 2)
   Covers: config · state · translations · audio ·
           achievements · settings · colors · panel nav
   ============================================================ */
...
// Default volumes (0–1 scale, stored as 0–100 in localStorage)
...
// ============================================================
// STATE — SETTINGS  (loaded from localStorage)
// ============================================================
...
// ============================================================
function canPlay(toggle) {
  return globalMusicOn && toggle.checked;
}

function playRespawnSound() {
  if (!canPlay(audioRespawn)) return;
  respawnSound.currentTime = START_TIMES.respawn;
  respawnSound.play().catch(() => {});
}

function playLavaDieSound() {
  if (!canPlay(audioLava)) return;
  lavaDieSound.currentTime = START_TIMES.lavaDie;
  lavaDieSound.play().catch(() => {});
}

function playJumpSound() {
  if (!canPlay(audioJump)) return;
  jumpSound.currentTime = START_TIMES.jump;
...
function playKillBrickSound() {
  if (!canPlay(audioBrick)) return;
  killBrickSound.currentTime = START_TIMES.killBrick;
  killBrickSound.play().catch(() => {});
}

function playAchievementSound() {
  if (!canPlay(audioAchievement)) return;
  achievementSound.currentTime = START_TIMES.achievement;
...
function resetAllData() {
  ...
  audioSettings = {
    respawn: true, lava: true, jump: true,
    walking: true, brick: true, ambient: true, achievement: true
  };

  // Re-sync all UI controls
...
/* ============================================================
   BLOCK JUMP  v1.2.0  —  app.js  (Part 2 / 2)
   Covers: event listeners · game functions · platform system
           · main game loop · initialisation
   ============================================================ */
...
graphicsModeToggle.addEventListener('click', () => {
  setGraphicsMode(graphicsMode === 'high' ? 'low' : 'high');
});
...
colorModeToggle.addEventListener('click', () => {
  setColorMode(colorMode === 'default' ? 'custom' : 'default');
});

cpBtn.addEventListener('input', () => {
  customColors.btn = cpBtn.value;
  document.getElementById('cp-btn-preview').style.background = cpBtn.value;
  saveCustomColors();
});
...
cpHud.addEventListener('input', () => {
  customColors.hud = cpHud.value;
  document.getElementById('cp-hud-preview').style.background = cpHud.value;
  saveCustomColors();
});
cpAccent.addEventListener('input', () => {
  customColors.accent = cpAccent.value;
  document.getElementById('cp-accent-preview').style.background = cpAccent.value;
  saveCustomColors();
});
...
function update(currentTime = 0) {
  ...
  // Update orange movement & blue fade
  platforms.forEach(p => {
